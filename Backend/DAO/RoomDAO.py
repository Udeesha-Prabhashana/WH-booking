...
class RoomDAO:
    ...
    @classmethod
    def addnewrooom(cls, price, facilities,hotel_id,maxPeople,title,room_number):
        try:
            conn = get_db_connection()
            cursor = conn.cursor()
            cursor.execute(
                "insert into room(price,facilities,hotel_id,maxPeople,title, room_number) value (%s, %s,%s, %s, %s , %s)",
                (price, facilities,hotel_id,maxPeople,title,room_number))
            # Get the room_id of the newly inserted room
            cursor.execute("SELECT LAST_INSERT_ID()")
            room_id = cursor.fetchone()[0]
            
            # Insert into room_numbers table
            cursor.execute(
                "INSERT INTO room_numbers (room_id, room_number) VALUES (%s, %s)",
                (room_id, room_number))
            conn.commit()
