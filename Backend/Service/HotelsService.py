...
class HotelService:
    ...
    def countByCity(cls , cities):
        responseData = cls.hotelDAO.countByCity(cities)
        print("countByCity", responseData)
        return responseData
    ...
    def addHotel(cls,data):
        responseData = cls.hotelDAO.addNewHotel(data.get('type'), data.get('city'), data.get('address'),data.get('title'),data.get('photo'),data.get('description'),data.get('cheapestPrice') )
        if responseData is not None:
            return "Hotel add successful!"
        else:
            return "Hotel add unsuccessful!"
