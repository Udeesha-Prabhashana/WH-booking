...
class RoomService:
    ...
    def addroom(cls,data):
        responseData = cls.roomDAO.addnewrooom(data.get('price'),data.get('facilities'),data.get('hotel_id'),data.get('maxPeople'),data.get('title'), data.get('room_number') )
    ...
    def deleteRoom(cls,room_id):
        responseData = cls.roomDAO.deleteRoom(room_id)
        if responseData is not None:
            return "Room deleted successful!"
        else:
            return "Room deleted unsuccessful!"
